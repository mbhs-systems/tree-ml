...
@test_safe
def test_for_dataset(data_dir):
    dataset_path = os.path.join(data_dir, 'data_road')
    training_labels_count = len(glob(os.path.join(dataset_path, 'training/gt_image_2/*_road_*.png')))
    training_images_count = len(glob(os.path.join(dataset_path, 'training/image_2/*.png')))
    testing_images_count = len(glob(os.path.join(dataset_path, 'testing/image_2/*.png')))

    assert not (training_images_count == training_labels_count == testing_images_count == 0),\
        'Kitti dataset not found. Extract Kitti dataset in {}'.format(dataset_path)
